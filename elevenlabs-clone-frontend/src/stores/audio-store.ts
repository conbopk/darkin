...
interface AudioState {
  ...
  skipForward: () => void;
  skipBackward: () => void;
  downloadAudio: (audio?: AudioInfo) => void;
}
...
export const useAudioStore = create<AudioState>((set, get) => ({
  ...
  downloadAudio: (audio) => {
    const audioToDownload = audio ?? get().currentAudio;

    if (!audioToDownload?.audioUrl) {
      console.warn("No audio available to download");
      return;
    }

    const link = document.createElement("a");
    link.href = audioToDownload.audioUrl;
    link.download = `${audioToDownload.title ?? "audio"}.wav`;
