...
import {HistoryPanel} from "~/components/speech-synthesis/history-panel";
  ...
  return (
      <>
        ...
        {isMobileScreen && isMobileMenuOpen && (
            <div
              className='fixed inset-0 z-40 bg-gray-200'
              onClick={toggleMobileMenu}
            />
        )}

        <div className={`fixed inset-x-0 bottom-0 z-50 transform transition-transform duration-300 ease-in-out lg:hidden ${isMobileMenuOpen ? "translate-y-0" : "translate-y-full"}`}>
          <div className='max-h-[80vh] overflow-y-auto rounded-t-xl bg-white p-5 shadow-lg'>
            ...
            {/*Tabs*/}
            <div className='relative mb-6 flex'>
              <div className='mb-4 bottom-0 left-0 right-0 border-b border-gray-200' />
              <button
                onClick={() => setActiveTab("settings")}
                className={`relative z-10 mr-4 pb-2 text-sm transition-colors duration-200 ${activeTab === "settings" ? "border-b-2 border-black text-black" : "text-gray-500 hover:text-gray-700"}`}
              >
