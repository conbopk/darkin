...
export function GenerateButton({
  ...
  const { downloadAudio } = useAudioStore();

  return (
      <div className={`flex w-full flex-col-reverse items-center gap-4 md:flex-row md:justify-between ${className}`}>
        {(showCredits ?? (showCharacterCount && characterCount !== undefined)) && (
            <div className='items-center flex w-full flex-wrap justify-between gap-2'>
              {showCredits && (
              ...
              {/*Character counter*/}
              {showCharacterCount && characterCount !== undefined && characterLimit !== undefined && (
